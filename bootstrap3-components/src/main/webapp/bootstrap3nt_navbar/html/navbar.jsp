...
<div class="${navbarClass}" role="navigation">
    <div class="container">
        <div class="navbar-header">
            ...
            <c:set var="siteName" value="${currentNode.resolveSite.properties['j:title'].string}"/>
            <jcr:nodeProperty var="siteLogo" node="${currentNode.resolveSite}" name="siteLogo"/>
            <c:choose>
                <c:when test="${!empty siteLogo.node}">
                    <a class="navbar-brand" href="<c:url value='${url.base}${currentNode.resolveSite.home.path}.html'/>">
        ...
        <div class="collapse navbar-collapse navbar-responsive-collapse">
            <c:choose>
                <c:when test="${fn:startsWith(renderContext.mainResource.path,'/templateSets/')}">
                    ...
                    <c:if test="${jcr:isNodeType(currentNode.resolveSite, 'bootstrap3mix:loginbarAdvanced')}">
                        <c:set var="ulClass" value="${currentNode.properties['ulClass'].string}"/>
                    </c:if>
                    <ul class="${ulClass}">
